...
#group data and nest to create seperate data frames
nested_data <- data %>%
  group_by(academic_year, district, school, grade_level) %>%
  nest()

#loop through each data frame to calculate effect size
data_new <- nested_data %>%
  mutate(effect_size = map(data, ~gap([[5]][[.x]], "white", "hispanic_latino")))
